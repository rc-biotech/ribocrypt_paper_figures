...
all_ids <- c("Organism (# Libraries)", "Inhibitor", "Tissues", "Cell Lines", "Filter pass rate", "File breakdown", "CDS' expressed(%) (> 100 psites)", "CDS psites (log10)")
...
expression <- rbindlist(lapply(all_allmerged$name, function(x) {
  df <- read.experiment(x, validate = FALSE)
  counts <- suppressWarnings(countTable(df))
...
names(psites) <-  names(cds_ratio_100) <- gsub("_", " ", gsub("all_merged-", "", all_allmerged$name[ordering]))
